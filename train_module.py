...
def auto_split(ref_model, pre_train_loader, pre_optimizer, pre_schedule, soft_split_all):
    ...
    for epoch in range(epoch_num):
        ...
        print('\rInitializing Env [%d/%d]  Loss: %.2f  IRM_Risk: %.2f  Reg: %.2f  Cnt: %d  Lr: %.2f'
              %(epoch, epoch_num, avg_risk, avg_irm_risk, sum(reg_list)/len(reg_list), cnt, pre_optimizer.param_groups[0]['lr']), end='',flush=True)

        if epoch > epoch_num//2 and cnt > 5 or epoch == epoch_num-1: #debug
...
def refine_split(bias_optimizer, bias_schedule, bias_classifier, classifier_train_loader, feature_extractor):
    ...
    for epoch in range(epoch_num):
        ...
        for data in classifier_train_loader:
            ...
            train_correct += batch_correct
            print('\rOptimizing [%d/%d]  Loss: %.2f  Acc: %.2f  Lr: %.3f'  % (epoch, epoch_num, bias_loss.item(), train_acc, bias_optimizer.param_groups[0]['lr']), end='', flush=True)
        ...
        if cnt > 5 or epoch == epoch_num-1: #debug
