...
class AttributorBase:
    ...
    def __call__(self, feature, output, class_idx=None, img_idx=None, classes=None, mask=None):
        if self.batch_mode:
            return self._call_batch_mode(feature, output, classes)
        return self._call_single(feature, output, class_idx, img_idx,mask) # evaluation mode?

    def _call_batch_mode(self, feature, output, classes):
        raise NotImplementedError

    def _call_single(self, feature, output, class_idx, img_idx,mask=None):
        raise NotImplementedError
...
class GradCamAttributor(AttributorBase):
    ...
    def _call_single(self, feature, output, class_idx, img_idx,mask=None):  # take the grad with respect to one class
        grads = torch.autograd.grad(
            output[img_idx, class_idx], feature, create_graph=True, retain_graph=True)[0]
        grads = grads.mean(dim=(2, 3), keepdim=True)
